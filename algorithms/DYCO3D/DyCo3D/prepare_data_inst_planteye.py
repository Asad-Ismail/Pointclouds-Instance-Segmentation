...
parser.add_argument('--src_dir', help='data directory contianing ply and labels', default='/media/asad/adas_cv_2/DyCo3D/dataset/planteye/train')
...
def f(fn):
    ...
    # convert classes to right format
    coords=coords.astype(np.float32)
    colors=colors.astype(np.float32)
    sem_labels=sem_labels.astype(np.float64)
    ins_label=ins_label.astype(np.float64)
