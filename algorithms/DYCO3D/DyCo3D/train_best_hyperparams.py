...
def train(train_loader, model, model_fn, optimizer, start_iter,end_epoch,scheduler, save_to_disc=True,data_name="planteye"):
    ...
    am_dict = {}
    model_fn_test = model_fn_decorator(test=True,device="cpu")
    model.train()
    ...
    for iteration, batch in tqdm(enumerate(train_loader, start_iter),total=cfg.max_iter):
        ...
        if save_to_disc:
            if iteration > cfg.prepare_epochs:
                sys.stdout.write(
                    ...
                    (iteration + 1, cfg.max_iter, scheduler.get_lr()[0], am_dict['loss'].val, am_dict['loss'].avg, am_dict['score_loss'].val, am_dict['score_loss'].avg,
                ...
                sys.stdout.write(
                    ...
                    (iteration + 1, cfg.max_iter, scheduler.get_lr()[0], am_dict['loss'].val, am_dict['loss'].avg,
                     data_time.val, data_time.avg, iter_time.val, iter_time.avg, remain_time=remain_time))
        ...
        if (iteration % cfg.test_epoch == 0  or iteration==cfg.max_iter-1) and save_to_disc and iteration!=0:
            ...
            if allap>best_ap:
                best_ap=allap
                best_cfg=cfg
            logger.info(f"Current AP_Plant: {allap}")
            logger.info(f"Best AP_Plant: {best_ap}")
            logger.info(f"Current Config {cfg}")
...
def find_best_hyperparams():
    ...
    hyperparams = {
        "cluster_radius": [0.01,0.02,0.03],
        "cluster_npoint_thre": [5,10,20,30],
    }
    ...
    for i, params in tqdm(enumerate(total_param_combinations, 1),total=len(total_param_combinations)):
        ...
        for param_index, param_name in enumerate(hyperparams):
            ...
            setattr(cfg, param_name,params[param_index] )
        # log the config
        logger.info(cfg)
