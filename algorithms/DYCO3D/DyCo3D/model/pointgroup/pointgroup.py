...
def model_fn_decorator(test=False,device="cuda"):
    #### config
    ...
    def test_model_fn(batch, model, epoch):
        coords = batch['locs'].to(device=device)              # (N, 1 + 3), long, cuda, dimension 0 for batch_idx
        voxel_coords = batch['voxel_locs'].to(device=device)  # (M, 1 + 3), long, cuda
        p2v_map = batch['p2v_map'].to(device=device)          # (N), int, cuda
        v2p_map = batch['v2p_map'].to(device=device)                     # (M, 1 + maxActive), int, cuda

        coords_float = batch['locs_float'].to(device=device)  # (N, 3), float32, cuda
        feats = batch['feats'].to(device=device)              # (N, C), float32, cuda

        batch_offsets = batch['offsets'].to(device=device)    # (B + 1), int, cuda
