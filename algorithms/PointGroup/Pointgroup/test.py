...
def test(model, model_fn, data_name, epoch):
    ...
    if data_name == 'planteye':
        ...
        dataset = data.planteye_inst.Dataset(start_iter=0,val=True)
        #dataset.trainLoader()
        dataset.valLoader()
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(dataloader):
            ...
            if data_name == 'planteye':
                test_scene_name = dataset.val_file_names[int(batch['id'][0])].split('/')[-1][:-17]
            else:
            ...
            ##### print
            logger.info("instance iter: {}/{} point_num: {} ncluster: {} time: total {:.2f}s inference {:.2f}s save {:.2f}s".format(batch['id'][0] + 1, len(dataset.val_files), N, nclusters, end, end1, end3))
