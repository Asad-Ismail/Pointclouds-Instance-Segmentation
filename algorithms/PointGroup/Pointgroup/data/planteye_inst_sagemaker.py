...
class Dataset:
    def __init__(self, training_dir,validation_dir,num_gpus,start_iter=0, test=False):
    ...
    def trainLoader(self):
        train_file_names = sorted(glob.glob(os.path.join(self.training_dir, '*' + self.filename_suffix)))
        ...
        if self.num_gpus > 1:
            ...
            images_per_gpu = self.batch_size // self.num_gpus
    ...
    def valLoader(self):
        val_file_names = sorted(glob.glob(os.path.join(self.validation_dir, '*' + self.filename_suffix)))
    ...
    def testLoader(self):
        self.test_file_names = sorted(glob.glob(os.path.join(self.validation_dir, '*' + self.filename_suffix)))
    ...
    def trainMerge(self, id):
        ...
        for i, idx in enumerate(id):
            ...
            #mins=xyz_origin.min(0)
            
            #maxs=xyz_origin.max(0)
            ...
            xyz = xyz[valid_idxs]
            rgb = rgb[valid_idxs].astype(np.float32)
            label = label[valid_idxs]
            instance_label = self.getCroppedInstLabel(instance_label, valid_idxs).astype(np.double)
